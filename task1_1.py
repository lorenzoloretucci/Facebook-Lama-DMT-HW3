...
# python lama/task1_1.py  --lm "bert" --t path_mashed_sentences
...
def get_ranking(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 10, print_generation=True):
    ...
    return  experiment_result['topk'][0]["token_word_form"]
...
def main(args):
    ...
    texts = args.text  # PATH CONTAINING OUR MASKED SENTENCES
    f = codecs.open(texts, 'r')  # READ SENTENCE FROM FILE
    for text in f:
        sentences = [text]
        out = ""
...
mask_val = []  # Get the masked entity from file
with open(path_true_mask, "r") as f: 
    for line in f:
        mask_val.append(line.strip("\n"))

label_val =[]  # Get labels from file
...
for i in range(len(results)):
    if results[i].lower() == mask_val[i].lower(): # Results contains the predicted WORD. # check if the predicted word matches the true mention
        RES.append("SUPPORTS")
    else:
        RES.append("REFUTES")
